...
startSheet.innerHTML += "::-moz-scrollbar { width: 12px; } ::-moz-scrollbar-track { -moz-box-shadow: inset 0 0 6px rgba(0,0,0,0.3); border-radius: 10px; } ::-moz-scrollbar-thumb { border-radius: 10px; background: ";
startSheet.innerHTML += "rgb(".concat(r1,",",g1,",",b1,"); -moz-transition: background 3s ease-in-out;}");

startSheet.innerHTML += "::-o-scrollbar { width: 12px; } ::-o-scrollbar-track { -o-box-shadow: inset 0 0 6px rgba(0,0,0,0.3); border-radius: 10px; } ::-o-scrollbar-thumb { border-radius: 10px; background: ";

startSheet.innerHTML += "rgb(".concat(r1,",",g1,",",b1,"); -o-transition: background 3s ease-in-out;}");
...
for (i = 0; i < steps; i++) {
	...
	if( $(window).scrollTop() >= scx && $(window).scrollTop() < ubound){
		...
		gstr = gsteps[i];
		bstr = bsteps[i];
		var final = start.concat(rstr,",",gstr,",",bstr,");} body, html{overflow: visible;} ::-moz-scrollbar-thumb {background: rgb(",rstr,",",gstr,",",bstr,");} body, html{overflow: visible;} ::-o-scrollbar-thumb {background: rgb(",rstr,",",gstr,",",bstr,");} body, html{overflow: visible;}");
